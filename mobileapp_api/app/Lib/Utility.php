...
class Utility
{
    ...
    function uploadAMultipartFileIntoFolder($param,$ext=null,$folder)
    {
...
// Close the cURL session
    ...
    function ifWordFound($word){




        $keywords = array("nudity","suggestive");
        $found = false;
    ...
    static function removeMainUrl($url){
...
// Parse the URL
        ...
        pr($parsedUrl);
        die();
// Check if the host is set and not empty
        if (isset($parsedUrl['host']) && !empty($parsedUrl['host'])) {
            // Split the host by ".com" and get the part after ".com"
            $parts = explode('.com', $parsedUrl['host']);
    ...
    public static function cropImage($inputImage){
...
// Load the image
            $sourceImage = imagecreatefrompng($inputImage); // Load a PNG image
...
// Crop the source image to 100x100 and copy it to the destination image
            imagecopyresampled($destinationImage, $sourceImage, 0, 0, 0, 0, 100, 100, $sourceWidth, $sourceHeight);

// Save or output the cropped image as PNG
            $outputImage = UPLOADS_FOLDER_URI . "/images/" . $file_name . ".png"; // Change this to your desired output image path
            imagepng($destinationImage, $outputImage); // Save the cropped image as PNG
...
// Clean up
            ...
            $sourceImage = imagecreatefromjpeg($inputImage); // Change the function according to your image format
...
// Save or output the cropped image
            $outputImage = UPLOADS_FOLDER_URI . "/images/" . $file_name . ".jpg";  // Change this to your desired output image path
            imagejpeg($destinationImage, $outputImage); // Change the function according to your desired image format
    ...
    static function getCloudFrontUrl($url,$before_string){
        ...
        if($aws)
        {
            ...
            if (strpos($parsed_url['host'], '.com') !== false) {
                ...
                if($string){
                   $url =  str_replace("tictic-video/", "", $url);
